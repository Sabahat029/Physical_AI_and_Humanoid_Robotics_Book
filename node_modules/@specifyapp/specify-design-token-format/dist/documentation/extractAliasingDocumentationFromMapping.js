import { isArrayOfTypesMapping, isDiscriminatedUnionOfTypesMapping, isMapOfTypesMapping, isPrimitiveTypesMapping, isTokenTypeTypesMapping, isTupleOfTypesMapping, isUnionOfTypesMapping, } from '../definitions/internals/tokenTypesMapping.js';
...
function makePadding(depth) {
    return '  '.repeat(atLeastZero(depth));
}
export function extractAliasingDocumentationFromMapping(tokenTypesMapping, depth = 1) {
    return Object.entries(tokenTypesMapping).reduce((acc, [key, value]) => {
        ...
        if (isMapOfTypesMapping(currentObject)) {
            const inner = Object.entries(currentObject._mapOf).reduce((acc, [k, v], i, xs) => {
                ...
                acc += `\n${makePadding(depth)}${k}: ${extractAliasingDocumentationFromMapping(v, depth + 1)}${!isTail ? ',' : ''}`;
                if (isTail) {
                    if (xs.length > 1) {
                        acc += `\n`;
                    }
        ...
        if (isArrayOfTypesMapping(currentObject)) {
            return `Array<${extractAliasingDocumentationFromMapping(currentObject._arrayOf[0], // We expect _arrayOf to be an array of one element
            depth)}>`;
        }
