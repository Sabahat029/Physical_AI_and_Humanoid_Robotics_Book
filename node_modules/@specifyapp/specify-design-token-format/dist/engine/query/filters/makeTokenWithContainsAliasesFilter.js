...
export function makeTokenWithContainsAliasesFilter(withContainsAliases) {
    if (withContainsAliases === undefined)
        return returnTrue;
    return function filter(tokenState) {
        ...
        switch (level) {
            case 'mode': {
                shouldReturnOnLevel = aliases.some(alias => alias.from.mode !== null && alias.from.valuePath.length === 0);
                break;
            }
            case 'value': {
                shouldReturnOnLevel = aliases.some(alias => alias.from.mode !== null && alias.from.valuePath.length !== 0);
