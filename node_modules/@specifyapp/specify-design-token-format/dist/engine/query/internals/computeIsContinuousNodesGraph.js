export function computeIsContinuousNodesGraph(nodesLike) {
    // We consider a graph being continuous if empty or with only one node
    if (nodesLike.length === 0 || nodesLike.length === 1) {
        return true;
    }
    const [nodesAtRootLevel, nodesAtOtherLevels] = nodesLike.reduce((acc, node) => {
    ...
    const hasMultipleRoots = sortedByShallowerWithoutRootNodes.some(node => node.path.length === smallerPathLength);
    if (hasMultipleRoots) {
        return false;
    }
