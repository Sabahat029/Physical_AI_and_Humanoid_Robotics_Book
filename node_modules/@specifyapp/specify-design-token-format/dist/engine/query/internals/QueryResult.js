 ...
 */
export class QueryResult extends Array {
    #nodeTypesSet;
    #treeState;
    constructor(queryContext, treeState) {
        super(...queryContext.nodeStates);
    ...
    render() {
        ...
        const parentPathByChildString = new Map();
        ...
        for (const [parentPath, nodes] of groupedByRootNode.entries()) {
            ...
            nodes.forEach(child => {
                ...
                parentPath.length > 0 ? child.path.slice(parentPath.length) : child.path, child.getJSONProperties());
            });
            result.push({
    ...
    merge(dedupeFn) {
        ...
        const newTreeState = sortedNodesInResults
            ...
            const maybeNode = node instanceof CollectionState
            ...
            const isARootNode = parentPath.length > 0 && !allStringPathsInResults.includes(parentPath.toString());
            if (isARootNode) {
    ...
    getPaths(type) {
        return this.toArray().map(node => type === 'array' ? node.path.toArray() : node.path.toString());
    }
    /**
