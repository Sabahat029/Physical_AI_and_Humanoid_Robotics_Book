...
export function traverseRawTokenModeValue(localMode, modeValue, path) {
    ...
    traverseJSONValue(modeValue, (value, path) => {
...
export function parseRawTokenValue(value, path = TreePath.empty()) {
    if (value !== null &&
        typeof value === 'object' &&
        !Array.isArray(value) &&
        '$alias' in value &&
        '$mode' in value) {
        throw new SDTFError('SDTF_TOKEN_INVALID_MODE', `Cannot get value of token "${value.$alias}" with mode "${value.$mode}" - Top level aliases cannot have a $mode.`);
    }
    ...
    return Object.entries(value)
        ...
        .reduce((acc, { aliasParts, primitiveParts }) => {
