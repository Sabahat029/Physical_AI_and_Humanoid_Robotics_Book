export function computeDeepModesResolvability(tokenStringPath, analyzedTokens) {
    ...
    analyzedToken.analyzedValueAliasParts.forEach(part => {
        switch (part.type) {
            case 'topLevelAlias': {
                const resolvability = computeDeepModesResolvability(part.alias.path.toString(), analyzedTokens);
            ...
            case 'valueLevelAlias': {
                const deepResolvability = computeDeepModesResolvability(part.alias.path.toString(), analyzedTokens);
                const deepModeResolvability = Reflect.get(deepResolvability ?? {}, part.alias.targetMode) ?? false;
                const maybeModeResolvability = Reflect.get(modesResolvability, part.localMode);
                Reflect.set(modesResolvability, part.localMode, (maybeModeResolvability ?? true) && deepModeResolvability);
                break;
            }
        }
    });
