...
export function analyzeValueAliasPart(analyzedToken, aliasPart, treeNodeSet) {
    if (aliasPart.type === 'topLevelAlias' && aliasPart.alias.path.isEqual(analyzedToken.path)) {
        throw new SDTFError('SDTF_CIRCULAR_ALIAS_REFERENCE_FOUND', `Alias "${aliasPart.alias.path}" on token "${analyzedToken.path}" is referencing itself.`);
    }
    else if ((aliasPart.type === 'modeLevelAlias' || aliasPart.type === 'valueLevelAlias') &&
        aliasPart.alias.path.isEqual(analyzedToken.path) &&
        aliasPart.alias.targetMode === aliasPart.localMode) {
        throw new SDTFError('SDTF_CIRCULAR_ALIAS_REFERENCE_FOUND', `Alias "${aliasPart.alias.path}" on token "${analyzedToken.path}" is referencing itself in mode "${aliasPart.localMode}".`);
    }
    ...
    if (referencedToken) {
        ...
        if (referencedToken.modes) {
            ...
            referencedTokenModes = deepResolveAnalyzedTokenModes(referencedToken.analyzedValueAliasParts[0].alias.path.toString(), treeNodeSet);
        }
        switch (aliasPart.type) {
            case 'topLevelAlias': {
                ...
                const matchResult = analyzedToken.definition.matchTokenTypeAgainstMapping(referencedToken.$type, ValuePath.empty(), _discriminatorKeyPath => {
                    /* v8 ignore next 5 */
                    throw new SDTFError('SDTF_INTERNAL_DESIGN_ERROR', 'Discriminator key path should not be triggered on top level aliases');
                });
                if (!matchResult.success) {
                    throw new SDTFError('SDTF_INVALID_ALIAS_TYPE_REFERENCED', `Alias "${referencedToken.path}" on token "${analyzedToken.path}" is of type "${referencedToken.$type}" but should be of type "${matchResult.expectedType}"`);
                }
                break;
            }
            case 'modeLevelAlias':
            case 'valueLevelAlias': {
                ...
                if (isResolvable) {
                    const matchResult = analyzedToken.definition.matchTokenTypeAgainstMapping(referencedToken.$type, aliasPart.valuePath ?? ValuePath.empty(), discriminatorKeyPath => {
                        const computedPath = [aliasPart.localMode, ...discriminatorKeyPath];
                        return pickInObject(analyzedToken.$value, computedPath);
                    });
                    if (!matchResult.success) {
                        throw new SDTFError('SDTF_INVALID_ALIAS_TYPE_REFERENCED', `Alias "${referencedToken.path}" on token "${analyzedToken.path}" is of type "${referencedToken.$type}" but should be of type "${matchResult.expectedType}"`);
                    }
