...
import { parseRawToken } from './internals/parseRawToken.js';
import { parseRawCollection } from './internals/parseRawCollection.js';
import { parseRawGroup } from './internals/parseRawGroup.js';
...
export function analyzeTokenTree(tokenTree = {}) {
    try {
        tokenTree = JSONParseIfString(tokenTree);
    }
    catch (error) {
        throw new SDTFError('SDTF_INVALID_TOKEN_TREE_INPUT', 'Failed to parse tokenTree from JSON string input.');
    }
    try {
        const shallowParsed = validateDesignTokenTreeRootNode(tokenTree);
        const analyzedTokens = new TreeNodeSet();
        const analyzedCollections = new TreeNodeSet();
        const analyzedGroups = new TreeNodeSet();
        traverseTokenTree(shallowParsed, {
            onToken: (path, token) => {
                // We validate at runtime that the array is only an array of string
                analyzedTokens.add(parseRawToken(new TreePath(path), token));
            },
            onCollection: (path, collectionProperties) => {
                analyzedCollections.add(
                // We validate at runtime that the array is only an array of string
                parseRawCollection(new TreePath(path), collectionProperties));
            },
            onGroup: (path, groupProperties) => {
                // We validate at runtime that the array is only an array of string
                analyzedGroups.add(parseRawGroup(new TreePath(path), groupProperties));
            },
        });
        /* ------------------------------------------
           Tokens
        ...
        try {
            ...
            if (error instanceof RangeError) {
                throw new SDTFError('SDTF_CIRCULAR_ALIAS_REFERENCE_FOUND', `A circular alias reference was found in initial token tree.`);
            }
            throw error;
        }
        /* ------------------------------------------
        ...
        analyzedCollections.all.forEach(collection => {
            analyzedTokens.getChildrenOf(collection.path).forEach(token => {
                ...
                checkForTokenModesInCollection(token.path.toString(), tokenModes, collection.path.toString(), collection.allowedModes);
            });
        });
        ...
        analyzedCollections.all.forEach(collection => {
            analyzedCollections.getParentsOf(collection.path).forEach(parentCollection => {
                throw new SDTFError('SDTF_NESTED_COLLECTION', `Collection "${collection.path}" is nested in collection "${parentCollection.path}".`);
            });
        });
        return {
            analyzedTokens,
        ...
        if (error instanceof z.ZodError) {
            ...
            throw new SDTFError('SDTF_INVALID_TOKEN_TREE_INPUT', `Validation error ${messages.join(', ')}`);
        }
        throw error;
    }
}
