...
export class TokenValuePaths {
    ...
    addLeaf(path) {
        if (path.length < 1) {
            throw new SDTFError('SDTF_INTERNAL_DESIGN_ERROR', 'TokenValuePath leaf path must be of length 1 or greater');
        }
        const initialLength = path.length;
        while (path.length) {
            const stringPath = path.join('.');
            if (this.pathsMap.has(stringPath))
                return;
