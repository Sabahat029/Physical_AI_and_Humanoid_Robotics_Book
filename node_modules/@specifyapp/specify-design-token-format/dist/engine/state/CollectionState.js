import { specifyDesignTokenValueModeSchema, } from '../../definitions/index.js';
...
export class CollectionState extends TreeNodeState {
    isToken = false;
    isGroup = false;
    isCollection = true;
    #parentCollection;
    #allowedModes = [];
    constructor(treeState, { path, name, $description, $extensions, allowedModes }) {
        super(treeState, {
            path,
            name,
            $description,
    ...
    renameMode(fromMode, toMode) {
        ...
        if (this.#allowedModes.includes(parsedToMode)) {
            throw new SDTFError('SDTF_MODE_RENAME_FAILED', `Collection "${this.path.toString()}" tried to rename mode "${fromMode}" to "${parsedToMode}", but "${parsedToMode}" is already in the allowed modes: ${this.#allowedModes
                .map(c => `"${c}"`)
                .join(',')}.`);
        }
        const index = this.#allowedModes.indexOf(fromMode);
        if (index === -1) {
            throw new SDTFError('SDTF_MODE_RENAME_FAILED', `Collection "${this.path.toString()}" tried to rename mode "${fromMode}" to "${parsedToMode}", but "${fromMode}" is not in the allowed modes: ${this.#allowedModes
                .map(c => `"${c}"`)
                .join(',')}.`);
        }
    ...
    deleteMode(mode) {
        ...
        if (index === -1) {
            throw new SDTFError('SDTF_MODE_DELETE_FAILED', `Collection "${this.path.toString()}" tried to delete mode "${mode}", but "${mode}" is not in the allowed modes: ${this.#allowedModes
                .map(c => `"${c}"`)
                .join(',')}.`);
        }
        if (this.#allowedModes.length - 1 === 0) {
            throw new SDTFError('SDTF_MODE_DELETE_FAILED', `Collection "${this.path.toString()}" tried to delete mode "${mode}", but it is the last mode of the collection.`);
        }
    ...
    move(toPath) {
        ...
        if (toPathMaybeCollectionState) {
            throw new SDTFError('SDTF_MOVE_FAILED', `Node "${toPath}" is a collection. Target a group or the root node "[]".`);
        }
        if (!isMovingToRootLevel && !toPathMaybeGroupState) {
            const maybeTokenState = this.treeState.getTokenState(toPath);
            if (maybeTokenState) {
                throw new SDTFError('SDTF_MOVE_FAILED', `Node "${toPath}" is a token. Target a group or the root node "[]".`);
            }
            throw new SDTFError('SDTF_TREE_NODE_NOT_FOUND', `Node "${toPath}" does not exist.`);
        }
        if (this.path.isRootOf(toPath)) {
            throw new SDTFError('SDTF_MOVE_FAILED', `Node "${toPath}" is a child of the current node "${this.path}".`);
        }
        if (toPathParentCollectionState) {
            throw new SDTFError('SDTF_MOVE_FAILED', `Nesting collection is not allowed. Node "${toPath}" include a collection as parents at path "${toPathParentCollectionState.path}".`);
        }
