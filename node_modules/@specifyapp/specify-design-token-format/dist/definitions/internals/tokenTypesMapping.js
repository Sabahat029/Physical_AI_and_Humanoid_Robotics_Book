...
export function matchTokenTypeAgainstMapping(type, mapping, path, getDiscriminatorValue = () => undefined, originalPath = path) {
    // TODO: can be better
    ...
    if (isMapOfTypesMapping(mapping)) {
        ...
        return matchTokenTypeAgainstMapping(type, mapping._mapOf[selector], new ValuePath(tail), getDiscriminatorValue, originalPath);
    }
    if (isArrayOfTypesMapping(mapping)) {
        return matchTokenTypeAgainstMapping(type, mapping._arrayOf[0], new ValuePath(tail), getDiscriminatorValue, originalPath);
    }
    if (isUnionOfTypesMapping(mapping)) {
        const result = mapping._unionOf.reduce((acc, m) => {
            if (acc.success)
                return acc;
            let localSuccess = false;
            let expectedType = acc.expectedType && acc.expectedType.length > 0
                ? `${acc.expectedType}, `
                : '';
            try {
                const result = matchTokenTypeAgainstMapping(type, m, path, getDiscriminatorValue, originalPath);
                if (result.success) {
                    localSuccess = true;
                }
                if (!result.success) {
    ...
    if (isDiscriminatedUnionOfTypesMapping(mapping)) {
        ...
        if (discriminator === undefined)
            throw new Error('Expected "discriminator" to be defined');
        ...
        if (mapOfTypes === undefined)
            throw new Error('Expected "mapOfTypes" to be defined');
        return matchTokenTypeAgainstMapping(type, mapOfTypes, path, getDiscriminatorValue, originalPath);
    }
    if (isTupleOfTypesMapping(mapping)) {
        if (selector === undefined)
            throw new Error('Expected "selector" to be defined');
        const numberSelector = typeof selector === 'string' ? parseInt(selector, 10) : selector;
        if (isNaN(numberSelector))
            throw new Error('Expected "selector" to be a number');
        return matchTokenTypeAgainstMapping(type, mapping._tuple[numberSelector], 
        // TODO: Used 3 times
        new ValuePath(tail), getDiscriminatorValue, originalPath);
    }
    ...
    if (isTokenTypeTypesMapping(mapping)) {
        if (path.length > 0)
            throw new Error('Expected "path" to be empty to resolve a token type');
...
export function isMapOfTypesMapping(data) {
...
export function isArrayOfTypesMapping(data) {
    if (typeof data !== 'object' || data === null)
        return false;
    if (!('_arrayOf' in data))
        return false;
...
export function isUnionOfTypesMapping(data) {
    if (typeof data !== 'object' || data === null)
        return false;
    if (!('_unionOf' in data))
        return false;
...
export function isDiscriminatedUnionOfTypesMapping(data) {
    if (typeof data !== 'object' || data === null)
        return false;
    if (!('_discriminatedUnionOf' in data))
        return false;
...
export function isTupleOfTypesMapping(data) {
    if (typeof data !== 'object' || data === null)
        return false;
    if (!('_tuple' in data))
        return false;
...
export function isPrimitiveTypesMapping(data) {
    if (typeof data !== 'object' || data === null)
        return false;
    if (!('_primitive' in data))
        return false;
    // Managed by Zod validation on input
    // if (
...
export function isTokenTypeTypesMapping(data) {
    if (typeof data !== 'object' || data === null)
        return false;
    if (!('_tokenType' in data))
        return false;
    // Managed by Zod validation on input
...
const mapOfTypesMappingSchema = z
    .object({
    _mapOf: z.record(z.string(), z.lazy(() => tokenTypesMappingSchema)),
})
    .strict();
...
const discriminatedUnionOfTypesMappingSchema = z
    ...
    .strict()
    .refine(data => {
    if (!data._discriminator)
        return false;
    if (!data._discriminatedUnionOf || !Array.isArray(data._discriminatedUnionOf))
        return false;
...
const tokenTypeTypesMappingSchema = z
    ...
    .strict();
const tokenTypesMappingSchema = z.union([
...
export function validateTokenTypesMapping(data) {
