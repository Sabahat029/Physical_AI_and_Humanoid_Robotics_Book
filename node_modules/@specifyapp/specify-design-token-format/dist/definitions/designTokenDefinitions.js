import { z } from 'zod';
import { specifyJSONArrayDefinition, specifyJSONBooleanDefinition, specifyJSONNullDefinition, specifyJSONNumberDefinition, specifyJSONObjectDefinition, specifyJSONStringDefinition, } from './tokenTypes/_JSON.js';
import { specifyArcDegreeNumberDefinition, specifyIntegerNumberDefinition, specifyPercentageNumberDefinition, specifyPositiveIntegerNumberDefinition, specifyPositiveNumberDefinition, specifyRGBColorNumberDefinition, specifyZeroToOneDefinition, } from './tokenTypes/_numbers.js';
...
export const specifyDesignTokenTypeNames = Object.freeze(specifyDesignTokenDefinitions.map(definition => definition.type));
const designTokenDefinitionsMap = specifyDesignTokenDefinitions.reduce((acc, definition) => ({ ...acc, [definition.type]: definition }), {});
export function getDesignTokenDefinition(type) {
...
export const specifyAliasableDesignTokenSchema = z.discriminatedUnion('$type', specifyDesignTokenDefinitions.map(definition => definition.aliasableTokenZodSchema));
export function validateSpecifyAliasableDesignToken(maybeToken) {
    return specifyAliasableDesignTokenSchema.parse(maybeToken);
}
export const specifyNonAliasableDesignTokenSchema = z.discriminatedUnion('$type', specifyDesignTokenDefinitions.map(definition => definition.resolvedTokenZodSchema));
export function validateSpecifyNonAliasableDesignToken(maybeToken) {
...
export function validateSpecifyDesignTokenTypeName(typeName) {
