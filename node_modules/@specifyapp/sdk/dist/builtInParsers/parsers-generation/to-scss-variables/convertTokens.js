...
import { dimensionToCss, breakpointToCss, colorToCss, cubicBezierToCss, durationToCss, borderToCss, gradientToCss, gradientsToCss, fontToCss, fontWeightToCss, opacityToCss, radiiToCss, radiusToCss, shadowToCss, shadowsToCss, spacingToCss, spacingsToCss, textStyleToCss, stepsTimingFunctionToCss, transitionToCss, zIndexToCss, } from '../../../converters/css/index.js';
import { makeScssVariable } from '../../../converters/scss/utils/makeScssVariable.js';
import { dataOfToken } from '../../utils/template.js';
export function convertTokens(sdtfEngine, parserOptions) {
    ...
    const convertedTokensByType = tokens.reduce((acc, token) => {
        ...
        acc[token.type] += Object.entries(tempOutput).reduce((acc, [mode, value]) => {
            const variableName = renderVariable(tokenNameTemplate, dataOfToken(token, hasOnlyOneMode && !parserOptions.shouldRenderSingleMode ? undefined : mode));
            acc += `${typeof value === 'string' || typeof value === 'number'
