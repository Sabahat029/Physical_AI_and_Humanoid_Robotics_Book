...
import { dataOfToken, } from '../../../shared/to-css-custom-properties/template.js';
import { makeCSSAlias } from '../../../shared/to-css-custom-properties/makeCSSAlias.js';
export function textStyleToTailwindWithCssVariables(token, removeSingleMode, renderVariable) {
...
export function textStyleToTailwind(value) {
    ...
    const letterSpacing = !!value.letterSpacing
        ...
        : undefined;
    if (!!textColor)
        tokens.push({ value: textColor, type: 'textColor' });
    if (!!textIndent)
        tokens.push({ value: textIndent, type: 'textIndent' });
    if (!!fontSize)
        tokens.push({ value: fontSize, type: 'fontSize' });
    if (!!lineHeight)
        tokens.push({ value: lineHeight, type: 'lineHeight' });
    if (!!letterSpacing)
        tokens.push({ value: letterSpacing, type: 'letterSpacing' });
    return tokens;
}
