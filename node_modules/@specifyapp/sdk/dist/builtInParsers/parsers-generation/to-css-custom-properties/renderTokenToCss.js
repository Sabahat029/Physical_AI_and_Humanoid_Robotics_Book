...
import { dimensionToCss, dimensionValueToCss, } from '../../../converters/css/tokenTypes/dimension.js';
import { colorToCss, colorValueToCss } from '../../../converters/css/tokenTypes/color.js';
import { cubicBezierToCss, cubicBezierValueToCss, } from '../../../converters/css/tokenTypes/cubicBezier.js';
import { durationToCss, durationValueToCss } from '../../../converters/css/tokenTypes/duration.js';
import { breakpointToCss, breakpointValueToCss, } from '../../../converters/css/tokenTypes/breakpoint.js';
...
import { gradientsToCss, gradientsValueToCss, } from '../../../converters/css/tokenTypes/gradients.js';
import { fontToCss, fontValueToCss } from '../../../converters/css/tokenTypes/font.js';
import { fontWeightToCss, fontWeightValueToCss, } from '../../../converters/css/tokenTypes/fontWeight.js';
...
import { textStyleToCss, textStyleValueToCss, } from '../../../converters/css/tokenTypes/textStyle.js';
import { transitionToCss, transitionValueToCss, } from '../../../converters/css/tokenTypes/transition.js';
import { stepsTimingFunctionToCss, stepsTimingFunctionValueToCss, } from '../../../converters/css/tokenTypes/stepsTimingFunction.js';
...
import { DEFAULT_TOKEN_TEMPLATE, DEFAULT_TOKEN_NOT_IN_COLLECTION_TEMPLATE, renderVariable as renderCssVariable, } from '../../../converters/css/utils/template.js';
import { dataOfToken } from '../../utils/template.js';
export function renderTokenToCss(token, parserOptions, toolbox, addedVariablesStorage = {}) {
    const allowUnresolvable = parserOptions?.allowUnresolvable ?? false;
    if (!allowUnresolvable && !token.isFullyResolvable)
        return undefined;
    const renderVariable = data => renderCssVariable(data.collection
        ? parserOptions?.tokenNameTemplate ?? DEFAULT_TOKEN_TEMPLATE
        : parserOptions?.tokenNotInCollectionNameTemplate ??
            DEFAULT_TOKEN_NOT_IN_COLLECTION_TEMPLATE, data);
    const aliasStrategy = createAliasToVarStrategy({
        tokenNameTemplate: parserOptions?.tokenNameTemplate ?? DEFAULT_TOKEN_TEMPLATE,
        tokenNotInCollectionNameTemplate: parserOptions?.tokenNotInCollectionNameTemplate ?? DEFAULT_TOKEN_NOT_IN_COLLECTION_TEMPLATE,
    });
    ...
    if (!tempOutput)
        return undefined;
    const outputByMode = Object.entries(tempOutput).reduce((acc, [mode, value]) => {
    ...
    if (!parserOptions?.includeCoreTokensInScopes)
        return outputByMode;
    ...
    aliases.forEach(alias => {
        if (alias.status === 'unresolvable')
            return;
        if (doesTokenBelongToSameCollection(token, alias.tokenState))
            return;
        ...
        if (!valueByMode)
            return;
        const formattedValueByMode = Object.entries(valueByMode).reduce((acc, [mode, value]) => {
...
//# sourceMappingURL=renderTokenToCss.js.map