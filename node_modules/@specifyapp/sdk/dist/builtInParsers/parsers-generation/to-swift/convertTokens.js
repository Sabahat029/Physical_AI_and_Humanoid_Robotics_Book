...
export function convertTokens(tokens, tokenNameTemplate) {
    const outputTokens = {};
    const renderVariable = makeRenderer(tokenNameTemplate);
    for (const token of tokens) {
        for (const mode of token.modes) {
            const variableName = renderVariable(dataOfToken(token, token.modes.length === 1 ? undefined : mode));
            const wrapValue = (value) => {
                if (!value)
                    return;
                return { variableName, value };
            };
            const output = matchJsonValue({
