...
export const makeLineHeightRelativeHandler = async (previousDataBox, toolbox, parserOptions, _, _context) => {
    let sdtfEngine;
    switch (previousDataBox.type) {
        ...
        default: {
            throw new SpecifyError({
                ...
                publicMessage: `${previousDataBox.type} is not a valid input for the convert-dimension parser.`,
            });
        }
    }
...
export function makeLineHeightRelativeInner(engine, options, toolbox) {
    const query = getSdtfQuery(options?.applyTo);
    const populateWarningMessageCallback = (token) => {
            ...
            return value
                ...
                if (lineHeightUnit === 'px' && fontSizeUnit === 'px') {
                    ...
                    if (lineHeightUnit !== 'px' && lineHeightUnit !== 'rem' && lineHeightUnit !== '%') {
                        toolbox.populateMessage({
                            type: 'warning',
                            content: 'The lineHeight unit is not supported. Only px, rem and % are supported.',
...
//# sourceMappingURL=makeLineHeightRelative.js.map