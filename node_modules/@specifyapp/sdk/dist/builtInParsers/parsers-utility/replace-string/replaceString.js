...
export const replaceStringHandler = async (previousResult, toolbox, parserOptions, _) => {
    let sdtfEngine;
    ...
    if (!('all' in parserOptions) &&
        ...
        toolbox.populateMessage({
            type: 'warning',
            content: "Couldn't find a pattern in all, token, group or collection, so nothing will be replaced",
...
export function replaceStringInner(engine, options) {
    ...
    (options.applyTo ? engine.query.run(query) : engine.query.getAllNodeStates()).forEach(node => {
        if ('all' in options) {
            ...
            const result = node.name.replace(regexOfOption(options.collection), options.collection.replaceBy);
