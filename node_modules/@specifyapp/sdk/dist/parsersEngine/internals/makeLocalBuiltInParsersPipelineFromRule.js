...
import { getBuiltInParserDefinition, } from '../../builtInParsers/builtInParsers.js';
/**
 * Build a ParserFunction, executed locally, wrapped into a ParsersPipeline from a given Parser rule
 * @param rule
 * @param context
 */
export function makeLocalBuiltInParsersPipelineFromRule(rule, context) {
    const fns = rule.parsers.map((parser, i, xs) => {
        const parserFunction = getBuiltInParserHandler(parser.name);
        const parserDefinition = getBuiltInParserDefinition(parser.name);
        const shouldExecuteRemotely = parser.options?.shouldExecuteRemotely ?? false;
        const isLastParser = i === xs.length - 1;
        return (previousDataBox, toolbox) => {
            if (!parserDefinition.inTypes.includes(previousDataBox.type)) {
                ...
                return createRPCClient(context.personalAccessToken)
                    ...
                    if (!result) {
                        throw new SpecifyError({
                            ...
                            publicMessage: `RPC result not found for parser "${parser.name}${rule.name ? ` on rule "${rule.name}"` : ''}.`,
                        });
                    }
...
//# sourceMappingURL=makeLocalBuiltInParsersPipelineFromRule.js.map