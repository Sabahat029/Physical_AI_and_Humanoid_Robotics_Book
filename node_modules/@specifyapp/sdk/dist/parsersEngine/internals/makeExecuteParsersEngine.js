...
import { parsersEngineDataBoxSchema, } from '../definitions/parsersEngineDataBox.js';
 ...
 * @param dataBox
 */
export function makeExecuteParsersEngine(pipelineStates, executionContext, dataBox) {
    ...
    return async function execute() {
        ...
        if (dataBox.type === 'SDTF Engine') {
            ...
            if (!isOnlyRemotelyExecutedRules || executionContext.isRemote) {
                const result = await executionContext.resolveRepositoryTokenTree(dataBox.owner, dataBox.name);
                if (result.status === 'success') {
                    engine = createSDTFEngine(result.tokenTree);
                }
                else {
                    // Return prematurely if the repository could not be resolved
                    return new ParsersEngineResults(pipelineStates.map(state => ({
        ...
        const execResults = await Promise.allSettled(pipelineStates.map(state => {
            let input;
            if (state.isFromRule) {
                if (engine) {
                    input = {
                    ...
                    input = parsersEngineDataBoxSchema.parse(dataBox);
                }
            }
            else {
                if (engine) {
            ...
            return state.fn(input, state.toolbox).then(next => parsersEngineDataBoxSchema.parse(next, {
        ...
        for (let i = 0; i < execResults.length; i++) {
            ...
            if (result.status === 'rejected') {
                let errorKey = specifyErrors.PARSERS_ENGINE_UNKNOWN_ERROR.errorKey;
                ...
                if (result.reason instanceof z.ZodError) {
                    ...
                    errorKey = result.reason.errorKey;
                    content = result.reason.message;
                    /* v8 ignore next 3 */
                }
...
//# sourceMappingURL=makeExecuteParsersEngine.js.map