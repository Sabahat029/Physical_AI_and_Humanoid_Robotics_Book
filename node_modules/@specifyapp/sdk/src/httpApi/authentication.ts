...
export function makePersonalAccessTokenHeaderValue(personalAccessToken: unknown) {
...
export function generatePersonalAccessTokenHeader(personalAccessToken: string | undefined):
  | {}
  | {
  ...
  return {
    [SPECIFY_HTTP_API_AUTHORIZATION_HEADER_NAME]:
      makePersonalAccessTokenHeaderValue(personalAccessToken),
  };
}

export function extractPersonalAccessTokenFromHeaderValue(headerValue: string) {
  if (headerValue.startsWith('PAT ')) {
    return headerValue.slice(4);
  }
}

const lowercase_SPECIFY_HTTP_API_AUTHORIZATION_HEADER_NAME =
  SPECIFY_HTTP_API_AUTHORIZATION_HEADER_NAME.toLowerCase();
export function extractPersonalAccessTokenFromHeaders(
