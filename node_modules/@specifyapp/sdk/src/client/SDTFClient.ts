...
import { UpdaterFunction } from './updaters/definitions.js';
import { SpecifyRepository } from '../httpApi/index.js';
import { createParsersEngine } from '../parsersEngine/createParsersEngine.js';
import { ParserFunction } from '../parsersEngine/definitions/ParserFunction.js';
import { SDTFEngineDataBox } from '../parsersEngine/index.js';
import { BuiltInGenerationParserRule } from '../builtInParsers/builtInParserRule.js';

/**
 * The main Specify client to interact with Specify repositories and the SDTF token tree.
 */
export class SDTFClient {
  readonly #initialTokenTree: SpecifyDesignTokenFormat;
  ...
  constructor(repository: SpecifyRepository, tokenTree: SpecifyDesignTokenFormat) {
   ...
   * @param path
   */
  pick(path: Array<string>) {
    const hasTrailingWildcard = path[path.length - 1] === '*';
    const finalPath = new TreePath(hasTrailingWildcard ? path.slice(0, -1) : path);

    let maybeCollectionState: CollectionState | undefined;
    let maybeGroupState: GroupState | undefined;
    let maybeTokenState: TokenState | undefined;
    ...
    if (maybeCollectionState) {
      let tokens: SpecifyDesignTokenFormat = {};
      ...
      if (hasTrailingWildcard) {
        ...
        tokens = rest as SpecifyDesignTokenFormat;
      } else {
        tokens = {
   ...
   */
  query(query: SDTFQuery, dedupeFn?: MergeDedupeFn) {
   ...
   */
  update(...updaters: Array<UpdaterFunction>) {
   ...
   */
  withQuery(query: SDTFQuery): { update: (...updaters: Array<UpdaterFunction>) => SDTFClient } {
    const self = this;
   ...
   * @param fn
   */
  executeEngine(fn: (engine: SDTFEngine) => void) {
    fn(this.engine);
    return this;
  }
   ...
   * @param fn
   */
  forEachTokenState(fn: (tokenState: TokenState, engine: SDTFEngine) => void) {
   ...
   * @param fn
   */
  mapTokenStates<T>(fn: (tokenState: TokenState, engine: SDTFEngine) => T) {
   ...
   */
  getTokenState(path: Array<string>) {
   ...
   * @param fn
   */
  forEachCollectionState(fn: (collectionState: CollectionState, engine: SDTFEngine) => void) {
    this.engine.query
   ...
   * @param fn
   */
  mapCollectionStates<T>(fn: (collectionState: CollectionState, engine: SDTFEngine) => T) {
   ...
   * @param {Array<string>} path
   */
  getCollectionState(path: Array<string>) {
   ...
   * @param fn
   */
  forEachGroupState(fn: (groupState: GroupState, engine: SDTFEngine) => void) {
   ...
   * @param fn
   */
  mapGroupStates<T>(fn: (groupState: GroupState, engine: SDTFEngine) => T) {
   ...
   * @param {Array<string>} path
   */
  getGroupState(path: Array<string>) {
  ...
  forEachQueryResult(
    query: SDTFQuery,
    fn: (treeNodeState: SDTFNodeState, engine: SDTFEngine, queryResult: QueryResult) => void,
  ) {
  ...
  mapQueryResults<T>(
    query: SDTFQuery,
    fn: (treeNodeState: SDTFNodeState, engine: SDTFEngine, queryResult: QueryResult) => T,
  ) {
    const queryResult = this.engine.query.run(query);
   ...
   */
  registerView(name: string, query: SDTFQuery, shouldSetActive: boolean = false) {
   ...
   * @param name
   */
  setActiveView(name: string | null) {
   ...
   */
  createParsersPipelines(...parsersPipelines: Array<ParserFunction<SDTFEngineDataBox>>) {
    return createParsersEngine(
      parsersPipelines,
      {
        type: 'SDTF Engine',
   ...
   * const results = await executePipelines();
   */
  createParsersPipelinesFromRules(...parserRules: Array<BuiltInGenerationParserRule>) {
    return createParsersEngine(
      parserRules,
      {
        type: 'SDTF Engine',
