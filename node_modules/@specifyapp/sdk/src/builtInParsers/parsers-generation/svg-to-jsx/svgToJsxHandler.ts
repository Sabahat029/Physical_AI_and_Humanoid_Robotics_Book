import type { SvgToJsxParserDefinition } from './definition.js';
...
export const svgToJsxHandler: DeriveBuiltInParserHandlerFromDefinition<
...
> = async (previousDataBox, toolbox, parserOptions, outputOptions, _context) => {
  ...
  switch (previousDataBox.type) {
    ...
    case 'SDTF Engine': {
      vectorAssets = (
        await generateVectorDataBoxAndPullSvgsContent(previousDataBox.engine, toolbox)
      ).assets;
      break;
    }
    default:
      throw new SpecifyError({
        ...
        } is not a valid input for the svg-to-jsx parser.`,
      });
  }
  ...
  toolbox.populateOutput({
    ...
    files: vectorAssets
      .map(asset => {
        ...
        const svg = generateDefaultJSXTemplate(
          asset.vector,
          { path: asset.path, mode: asset.mode },
          {
            reactVersion,
            filePrefix,
            fileSuffix,
