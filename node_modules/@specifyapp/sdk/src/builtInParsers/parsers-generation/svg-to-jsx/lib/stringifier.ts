...
function stringifyStyle(style = {}) {
  ...
  return proprietyNames.reduce((accumulator, proprietyName) => {
    const propriety = Reflect.get(style, proprietyName);

    if (typeof propriety === 'string') {
...
export function stringify(node: Node | string): string {
  ...
  return `<${node.name}${attributes}>${node.children.map(stringify).join()}</${node.name}>`;
}
