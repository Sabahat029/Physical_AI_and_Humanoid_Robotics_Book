import { SpecifyBorderValue } from '@specifyapp/specify-design-token-format';
import { colorToSwift } from './color.js';
import type { MultipleConversionOutput } from '../convertTokens.js';
import { dimensionToSwift } from './dimension.js';

export function borderToSwift(
  border: SpecifyBorderValue,
  variableName: string,
): MultipleConversionOutput {
  const color = colorToSwift(border.color);

  if (!color) return [];

  if (border.rectangleCornerRadii) {
    return [
      {
        variableName,
        value: `RoundedRectangle(cornerRadius: ${dimensionToSwift(
          border.rectangleCornerRadii[0],
        )}).stroke(${color}, lineWidth: ${dimensionToSwift(border.width)})`,
        type: 'border',
      },
    ];
  }

  return [
    {
      variableName: `${variableName}Color`,
      value: color,
      type: 'color',
    },
    {
      variableName: `${variableName}Width`,
      value: dimensionToSwift(border.width),
      type: 'dimension',
    },
  ];
}
