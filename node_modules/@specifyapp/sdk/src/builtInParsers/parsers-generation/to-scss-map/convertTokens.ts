import { SDTFEngine, TokenState } from '@specifyapp/specify-design-token-format';
...
import { ToScssMapParserOptions } from './definition.js';

export function convertTokens(
  engine: SDTFEngine,
  basePath: string,
  parserOptions: Omit<
    Required<Exclude<ToScssMapParserOptions, undefined>>,
    ...
    switch (type) {
      case 'textStyle': {
        map = textStylesToScssMap(
          ...
          tokens as Array<TokenState<'textStyle'>>,
          parserOptions.tokenNameTemplate,
        );
        break;
      }
      ...
      case 'color': {
        map = colorsToScssMap(
          tokens as Array<TokenState<'color'>>,
          parserOptions.tokenNameTemplate,
        );
        break;
      }
      ...
      case 'breakpoint':
      case 'spacing': {
        map = dimensionsToScssMap(
          type,
          tokens as Array<
            TokenState<'dimension'> | TokenState<'breakpoint'> | TokenState<'spacing'>
          >,
          parserOptions.tokenNameTemplate,
        );
        break;
