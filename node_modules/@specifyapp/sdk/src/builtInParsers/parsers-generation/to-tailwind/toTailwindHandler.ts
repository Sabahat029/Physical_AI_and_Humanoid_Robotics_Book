...
import type { ToTailwindParserDefinition } from './definition.js';
import {
  createSDTFEngine,
  SDTFEngine,
...
export const toTailwindHandler: DeriveBuiltInParserHandlerFromDefinition<
...
> = async (previousDataBox, toolbox, parserOptions, outputOptions, _context) => {
  ...
  if (tokenStates.length === 0) {
    ...
    const tailwindPreset = tokensToTailwindPreset(
      tokenStates,
      {
        useCssVariable: parserOptions?.useCssVariable ?? false,
        cssVariableTemplate: {
          tokenNameTemplate:
            parserOptions?.cssVariableTemplate?.tokenNameTemplate ?? DEFAULT_TOKEN_TEMPLATE,
