...
export type jsonValue = null | boolean | string | number | object | Array<jsonValue>;
...
type jsonTokenWithMode = {
  value: jsonModeValue;
  nameExtension?: string;
};

type outputType = 'raw' | 'css';

export function tokenToJson(
  token: TokenState,
  toolbox: ParserToolbox,
  outputType: outputType,
): Array<jsonTokenWithMode> | undefined {
  ...
  if (token.modes.length === 0) return undefined;
  ...
  const output = token.modes.reduce((acc, mode) => {
    ...
    if (!converted || (Array.isArray(converted) && converted.length === 0)) return acc;
    ...
    return acc;
  }, new Map<string, jsonTokenWithMode>());

  return Array.from(output.values());
}

function wrapOutput(value: jsonValue | undefined) {
  return value === undefined ? undefined : { value };
}

const cssMatcher: jsonValueMatcher<jsonToken | Array<jsonToken>> = {
...
function tokenJsonValueToJson(token: TokenState, mode: string, output: 'css' | 'raw') {
  if (output === 'raw') {
    return wrapOutput(
      token.getJSONValue({
        resolveAliases: true,
    ...
    return convertedToken.map(({ value, nameExtension }) => {
      ...
      return {
        ...
        path: `${defaultPath === '' ? '' : defaultPath}${
          defaultPath !== '' && !!nameExtension ? SDTF_PATH_SEPARATOR : ''
        }${!!nameExtension ? token.name : ''}`,
