...
import type { ToJsonParserDefinition } from './definition.js';
...
import { tokensToJson } from './tokenToJson.js';

export const toJsonHandler: DeriveBuiltInParserHandlerFromDefinition<
...
> = async (previousDataBox, toolbox, parserOptions, outputOptions, _context) => {
  ...
  switch (previousDataBox.type) {
    ...
    default: {
      throw new SpecifyError({
        ...
        } is not a valid input for the to-json parser.`,
      });
    }
  }
  ...
  if (outputOptions?.type !== 'file')
    throw new SpecifyError({
      ...
      publicMessage: `The output type ${outputOptions?.type} is not supported by the to-json parser.`,
    });
  ...
  if (tokenStates.length === 0) {
    ...
    const outputType = parserOptions?.output ?? 'raw';

    const json = tokensToJson(tokenStates, toolbox, outputType);
