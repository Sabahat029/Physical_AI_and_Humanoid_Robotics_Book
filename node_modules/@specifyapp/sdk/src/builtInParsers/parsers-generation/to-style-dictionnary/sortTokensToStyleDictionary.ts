...
export type styleDictionaryStructure = {
  [k: string]: {
    [tokenName: string]: {
      value: any;
    };
...
export function sortTokens(type: TokenState['type'], tokens: Array<TokenState>) {
  return tokens.reduce((acc, tokenState) => {
    ...
    if (!tokenState.isFullyResolvable) return acc;
    ...
    return merge(
      acc,
      Object.entries(output).reduce((acc, [mode, value]) => {
        if (!value) return acc;
        ...
        return merge(
          acc,
          Array.isArray(value) && tokenState.type !== 'cubicBezier'
            ? {
...
export function sortTokensToStyleDictionary(
  type: TokenState['type'],
  tokens: Array<TokenState>,
): styleDictionaryStructure | undefined {
  switch (type) {
    case 'border':
    case 'breakpoint':
    case 'color':
...
export function sortTokenStatesToStyleDictionary(tokenStates: Array<TokenState>) {
  ...
  return Object.entries(tokenStatesByType).reduce((acc, [t, tokenStates]) => {
    const type = t as TokenState['type'];
