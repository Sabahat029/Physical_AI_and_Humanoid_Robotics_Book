...
import {
  SELECTOR_SEPARATOR,
  DEFAULT_INCLUDE_CORE_TOKENS_IN_SCOPES,
} from '../../shared/to-css-custom-properties/constants.js';
import {
...
import type { ToCssCustomPropertiesParserDefinition } from './definition.js';
import { renderTokenToCss } from './renderTokenToCss.js';

export const toCssCustomPropertiesHandler: DeriveBuiltInParserHandlerFromDefinition<
  ToCssCustomPropertiesParserDefinition
> = async (previousDataBox, toolbox, parserOptions, outputConfiguration, _context) => {
  ...
  const tokenStatesByCollection = groupBy(tokenStates, tokenState =>
    collections.find(collectionState =>
      tokenState.path.toString().includes(collectionState.path.toString()),
    )?.name
  ...
  if (Reflect.ownKeys(tokenStatesByCollection).length === 0) {
    ...
    const result = Object.entries(tokenStatesByCollection).reduce(
      (globalAcc, [collectionName, tokenStates]) => {
        ...
        const addedTokensStorage: { [mode: string]: { [variable: string]: true } } = {};

        if (collectionName === 'root') {
          const rootCSS =
            tokenStates.reduce((acc, tokenState) => {
              ...
              const resolvedTokenWithMode = renderTokenToCss(
                tokenState,
                finalParserOptions,
                toolbox,
                addedTokensStorage,
              ...
              if (!resolvedTokenWithMode) return acc;
          ...
          const cssByMode = tokenStates.reduce(
            (acc, tokenState) => {
              if (!allowUnresolvable && !tokenState.isFullyResolvable) {
                toolbox.populateMessage({
                  type: 'warning',
