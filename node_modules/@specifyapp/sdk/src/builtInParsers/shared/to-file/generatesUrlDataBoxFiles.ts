...
import { ParserToolbox, AssetDataBox } from '../../../parsersEngine/index.js';

type PartialUrlAssets = Array<Pick<AssetDataBox['assets'][number], 'url' | 'format' | 'provider'>>;

function unwrapFilesValue(files: SpecifyFontValue['files']) {
...
export function generateUrlDataBoxFiles(tokens: SpecifyDesignTokenFormat, toolbox: ParserToolbox) {
  ...
  return assetsTokens.reduce(
    (acc, tokenState) => {
      ...
      const resultsByMode = tokenState.matchJSONValueByType<PartialUrlAssets>(
        {
