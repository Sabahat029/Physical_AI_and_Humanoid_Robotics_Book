import { Ast, ValueWithMode, Value } from './tokenToAstToken.js';
...
type Primitive =
  | string
  | number
  | Array<string | number | Record<string, number | string | Array<string | number>>>
  | Record<string, string | number | Array<string | number>>;

type PrimitiveWithMode = {
  [mode: string]: Primitive;
};

const inspect = (str: Parameters<typeof nodeInspect>[0]) =>
  nodeInspect(str, {
    depth: Infinity,
...
export function sanitizeIdent(ident: string) {
...
export function generateTokens({
  ...
  moduleExport,
  tokensToUnquote,
}: Omit<commonParams, 'ident'> & {
  tokens: {
    [path: string]: PrimitiveWithMode | Primitive;
  };
  tokensToUnquote: Array<{ path: string; key?: string }>;
}) {
  let code = `/**
...
*/\n`;
  ...
  code += `${
    moduleExport === 'es6' ? 'export ' : ''
  }const ${TOKENS_VAR_NAME} = /** @type {const} */ (${inspect(tokens)})${
...
function unquoteToken(code: string, path: string, key: string | undefined): string {
  return code.replace(
    key
      ? new RegExp(
          `(['"]?${path.replaceAll('.', '\\.')}['"]?: {[^]*['"]?${key}['"]?: )["'](.*)['"]`,
...
function makePathsIdent(ident: string, isType: boolean = false) {
...
export function generateAllPaths({
  ...
  isTypescript,
  moduleExport,
}: Omit<commonParams, 'ident'> & { ast: Ast }) {
  let result = '';
  const allPaths: Array<string> = [];

  const typeIdents: Array<string> = [];

  Object.entries(ast.dataByType).forEach(
    ([
      ident,
      {
        metadata: { paths },
...
*/\n`;
      ...
      js += `${
        moduleExport === 'es6' ? 'export ' : ''
      }const ${varIdent} = /** @type {const} */ (${inspect(arrPaths)}${
        isTypescript ? ' as const' : ''
      });\n`;
...
*/\n`;
    ...
    js += `${
      moduleExport === 'es6' ? 'export ' : ''
    }const ${varIdent} = /** @type {const} */ (${inspect(paths)}${
      isTypescript ? ' as const' : ''
    });\n`;
...
function makeModesIdent(baseIdent: string, isType: boolean) {
...
* To use this type you can do: \`@type {import('path/to/myTokensFile').${typeIdent}}\`
*/
${moduleExport === 'es6' ? 'export ' : ''}const ${makeModesIdent(
    ident,
    false,
  )} = /** @type {const} */ (${inspect(modes)})${isTypescript ? ' as const' : ''};${
...
export function generateAllModes({
  ast,
  isTypescript,
  moduleExport,
}: Omit<commonParams, 'ident'> & {
  ...
  if (allModeUnion.length === 0) return result;
...
export function generateGetTokenByModeFunction({
  isTypescript,
  moduleExport,
}: Omit<commonParams, 'ident'>) {
...
*/\n`;
  ...
  return (
    jsDoc +
    `${moduleExport === 'es6' ? 'export ' : ''}function ${GET_TOKEN_BY_MODE_FN_NAME}${
      isTypescript ? typescriptGeneric : ''
    }(path${isTypescript ? ': Path' : ''}, mode${isTypescript ? ': Mode' : ''})${
      isTypescript ? ': Return' : ''
    } {
...
export function generateGetTokenWithModesFunction({
  isTypescript,
  moduleExport,
}: Omit<commonParams, 'ident'>) {
  const jsDoc = `/**
...
*/\n`;
  ...
  return (
    jsDoc +
    `${moduleExport === 'es6' ? 'export ' : ''}function ${GET_TOKEN_WITH_MODES_FN_NAME}${
      isTypescript ? typescriptGeneric : ''
    }(path${isTypescript ? ': Path' : ''})${
      isTypescript ? ': ' + TOKENS_TYPE_NAME + '[Path]' : ''
    } {
  if (!${TOKENS_VAR_NAME}[path]) {
    throw new Error(${pathError})
  }
...
export function generateGetTokensByTypeFunction({
  isTypescript,
  moduleExport,
}: Omit<commonParams, 'ident'>) {
  const jsDoc = `/**
...
*/\n`;
  ...
  return (
    jsDoc +
    `${moduleExport === 'es6' ? 'export ' : ''}function ${GET_TOKENS_BY_TYPE_FN_NAME}${
      isTypescript ? '<Type extends keyof ' + PATHS_BY_TYPE_TYPE_NAME + '>' : ''
    }(type${isTypescript ? ': Type' : ''}) {
...
export function generateWithModeFunction({
  isTypescript,
  moduleExport,
}: Omit<commonParams, 'ident'> & { ast: Ast }) {
  return `/**
 ...
 */
export function ${WITH_MODE_FN_NAME}${
    isTypescript ? '<Mode extends KeysOfUnion<Tokens[keyof Tokens]>>' : ''
  }(mode${isTypescript ? ': Mode' : ''}) {
  /**
...
function primitiveOfValue(
  { value, isCode }: Value,
  mode?: string,
): [Primitive, Array<string | undefined>] {
  if (!Array.isArray(value) && typeof value === 'object') {
    const toUnquoteTokens: Array<string> = [];
    return [
      Object.entries(value).reduce((acc, [key, value]) => {
        ...
        return acc;
      }, {} as { [key: string]: string | number | Array<string | number> }),
      toUnquoteTokens,
    ];
  } else {
...
function getPrimitiveValueOfAstValue(
  valueWithMode: ValueWithMode,
): [jsValue: Primitive | PrimitiveWithMode, toUnquoteKeys: Array<string | undefined>] {
  const modes = Object.keys(valueWithMode);

  let toUnquoteKeys: Array<string | undefined> = [];
...
export function generateAllParts({
  ...
  const tokensToUnquote: Array<{ path: string; key?: string }> = [];
  ...
  result += generateTokens({
    tokens: Object.values(ast.dataByType).reduce((acc, astData) => {
      ...
      return acc;
    }, {} as { [path: string]: PrimitiveWithMode | Primitive }),
    tokensToUnquote,
    isTypescript,
    moduleExport,
  });
...
type options = {
  isTypescript: boolean;
  exportStyle: commonParams['moduleExport'];
...
export function generateCode(
  ast: Ast,
  { isTypescript, exportStyle, codeToAppend, codeToPrepend }: options,
): string {
