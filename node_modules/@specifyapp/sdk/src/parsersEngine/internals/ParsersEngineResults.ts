...
import { FilesOutput, filesOutputFileSchema, ParserOutput } from '../definitions/parserOutput.js';
import type {
...
export class ParsersEngineResults<R extends Array<ParsersEngineResult>> {
  #results: R;
  ...
  logErrorMessages() {
    this.#results.forEach(result => {
      result.errorMessages.forEach(message =>
        console.log(`${result.pipelineName}: ${message.content} - ${message.errorKey}`),
      );
    });
  }
  ...
  logInformationMessages() {
    this.#results.forEach(result => {
      result.informationMessages.forEach(message =>
        console.log(`${result.pipelineName}: ${message.content}`),
      );
    });
  }
      ...
      if (state.next) {
        ...
        if (withNextContent) {
          switch (state.next.type) {
            ...
            case 'vector': {
              console.log(
                `   [vector] ${state.next.assets.map(file => file.path).join(',\n         ')}`,
              );
              break;
            }
            case 'bitmap': {
              console.log(
                `   [bitmap] ${state.next.assets.map(file => file.path).join(',\n         ')}`,
              );
              break;
            }
            case 'asset': {
              console.log(
                `   [asset] ${state.next.assets.map(file => file.path).join(',\n         ')}`,
              );
              break;
            }
          }
   ...
   * @param fn
   */
  mapOutput(fn: (output: ParserOutput | null) => ParserOutput | null | void) {
   ...
   */
  mapFiles(fn: (file: FilesOutput['files'][number]) => FilesOutput['files'][number] | void) {
    for (let x = 0; x < this.#results.length; x++) {
   ...
   */
  async writeToDisk(directoryPath?: string) {
      ...
      fsResults.reduce<{
        ...
        (acc, curr) => {
          if (curr.status === 'rejected') {
            acc.errors.push(curr.reason as SpecifyError);
          } else {
