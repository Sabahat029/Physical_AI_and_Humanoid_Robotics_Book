 ...
 */
export function shadowToCss<UnresolvableReturn = never>(
  ...
  return (tokenState: TokenState<'shadow'>) =>
    tokenState
      ...
      .mapTopLevelValue(value =>
        value
          .mapRawValue(rawValue =>
            rawShadowToCss(rawValue, aliasStrategy, unresolvableAliasStrategy),
          )
 ...
 */
export function rawShadowToCss<UnresolvableReturn = never>(
  shadow: RawValueSignature<'shadow'>,
  aliasStrategy: CssResolvableAliasStrategy,
  unresolvableAliasStrategy: CssUnresolvableAliasStrategy<UnresolvableReturn> = DEFAULT_UNRESOLVABLE_STRATEGY,
) {
  return `${shadow.type.unwrapValue() === 'inner' ? 'inset ' : ''}${unwrapInnerValue(
    shadow.offsetX.mapPrimitiveValue(d => rawDimensionToCss(d)),
    aliasStrategy,
    unresolvableAliasStrategy,
  )} ${unwrapInnerValue(
    shadow.offsetY.mapPrimitiveValue(d => rawDimensionToCss(d)),
    aliasStrategy,
    unresolvableAliasStrategy,
  )} ${unwrapInnerValue(
    shadow.blurRadius.mapPrimitiveValue(d => rawDimensionToCss(d)),
    aliasStrategy,
    unresolvableAliasStrategy,
  )} ${unwrapInnerValue(
    shadow.spreadRadius.mapPrimitiveValue(d => rawDimensionToCss(d)),
    aliasStrategy,
    unresolvableAliasStrategy,
  )} ${unwrapInnerValue(
    shadow.color.mapPrimitiveValue(c => rawColorToCss(c, aliasStrategy)),
    aliasStrategy,
    unresolvableAliasStrategy,
  )}`;
}
 ...
 */
export function shadowValueToCss(shadow: SpecifyShadowValue) {
  return `${shadow.type === 'inner' ? 'inset ' : ''}${dimensionValueToCss(
    shadow.offsetX,
  )} ${dimensionValueToCss(shadow.offsetY)} ${dimensionValueToCss(
    shadow.blurRadius,
  )} ${dimensionValueToCss(shadow.spreadRadius)} ${colorValueToCss(shadow.color) ?? ''}`;
}
