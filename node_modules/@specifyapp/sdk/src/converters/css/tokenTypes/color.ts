 ...
 * E.g: `#ff00ff`
 */
export function colorToCss<UnresolvableReturn = never>(
  ...
  return (tokenState: TokenState<'color'>) =>
    tokenState
      ...
      .mapTopLevelValue(value =>
        value
          .mapRawValue(rawValue =>
            rawColorToCss(rawValue, aliasStrategy, unresolvableAliasStrategy),
          )
 ...
 */
export function rawColorToCss<UnresolvableReturn = never>(
  ...
  if (matchIsRawColorHex(color)) {
    ...
    return unwrapInnerValue(
      color.hex.mapPrimitiveValue(hex => renderHex(hex, alpha)),
      aliasStrategy,
    );
  } else if (matchIsRawColorRgb(color)) {
    return `rgb${alpha < 1 ? 'a' : ''}(${unwrapInnerValue(
      color.red,
      aliasStrategy,
      ...
      aliasStrategy,
      unresolvableAliasStrategy,
    )}, ${unwrapInnerValue(color.blue, aliasStrategy, unresolvableAliasStrategy)}${
      alpha < 1 ? ', ' + alpha : ''
    })`;
  } else if (matchIsRawColorHsl(color)) {
    return `hsl(${unwrapInnerValue(
      color.hue,
      aliasStrategy,
      unresolvableAliasStrategy,
    )}, ${unwrapInnerValue(
      color.saturation.mapPrimitiveValue(s => `${s}%`),
      aliasStrategy,
      unresolvableAliasStrategy,
    )}, ${unwrapInnerValue(
      color.lightness.mapPrimitiveValue(l => `${l}%`),
      aliasStrategy,
      unresolvableAliasStrategy,
    )}${alpha < 1 ? ' / ' + alpha : ''})`;
  } else if (matchIsRawColorHsb(color)) {
    return `hsv(${unwrapInnerValue(
      color.hue,
      aliasStrategy,
      unresolvableAliasStrategy,
    )}, ${unwrapInnerValue(
      ...
      color.brightness.mapPrimitiveValue(s => `${s}%`),
      aliasStrategy,
      unresolvableAliasStrategy,
    )}${alpha < 1 ? ' / ' + alpha : ''})`;
  } else if (matchIsRawColorLab(color)) {
    return `lab(${unwrapInnerValue(
      color.lightness.mapPrimitiveValue(s => `${s}%`),
      aliasStrategy,
      unresolvableAliasStrategy,
    )} ${unwrapInnerValue(
      color.aAxis,
      aliasStrategy,
      unresolvableAliasStrategy,
    )} ${unwrapInnerValue(color.bAxis, aliasStrategy, unresolvableAliasStrategy)}${
      alpha < 1 ? ' / ' + alpha : ''
    })`;
  } else if (matchIsRawColorLch(color)) {
    return `lch(${unwrapInnerValue(
      color.lightness.mapPrimitiveValue(s => `${s}%`),
      aliasStrategy,
      unresolvableAliasStrategy,
    )} ${unwrapInnerValue(
      ...
      aliasStrategy,
      unresolvableAliasStrategy,
    )} ${unwrapInnerValue(color.hue, aliasStrategy, unresolvableAliasStrategy)}${
      alpha < 1 ? ' / ' + alpha : ''
    })`;
  } else {
 ...
 */
export function colorValueToCss(color: SpecifyColorValue): string {
  ...
  switch (color.model) {
    ...
    case 'lch': {
      return `lch(${color.lightness}% ${color.chroma} ${color.hue}${
        alpha < 1 ? ' / ' + alpha : ''
      })`;
    }
    case 'lab': {
      return `lab(${color.lightness}% ${color.aAxis} ${color.bAxis}${
        alpha < 1 ? ' / ' + alpha : ''
      })`;
    }
  }
...
function renderHex(hex: string, alpha: number) {
