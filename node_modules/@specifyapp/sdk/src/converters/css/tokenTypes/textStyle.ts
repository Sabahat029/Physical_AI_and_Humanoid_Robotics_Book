...
```
 */
export function textStyleToCss<UnresolvableReturn = never>(
  ...
  return (tokenState: TokenState<'textStyle'>) =>
    tokenState
      ...
      .mapTopLevelValue(value =>
        value
          .mapRawValue(rawValue =>
            rawTextStyleToCss(rawValue, aliasStrategy, unresolvableAliasStrategy),
          )
          .mapResolvableModeLevelAlias(aliasStrategy)
          .mapUnresolvableModeLevelAlias(alias => unresolvableAliasStrategy(tokenState, alias))
          .unwrap(),
      )
      .unwrap() as { [mode: string]: ReturnType<typeof textStyleValueToCss> };
}

export function modeLevelAliasTextStyleToVariables(
  alias: ResolvableModeLevelAlias | ResolvableTopLevelAlias,
  renderAlias: TemplateRenderer,
  textStyle: RawValueSignature<'textStyle'>,
  withCssAlias = true,
) {
  const targetAlias = renderAlias(
    dataOfToken(
      alias.tokenState,
      alias instanceof ResolvableModeLevelAlias ? alias.targetMode : undefined,
    ),
  );
...
export function modeLevelUnresolvableAliasTextStyleToVariables(
  alias: UnresolvableModeLevelAlias,
  renderAlias: TemplateRenderer,
  withCssAlias = true,
) {
 ...
 */
export function rawTextStyleToCss<UnresolvableReturn = never>(
  textStyle: RawValueSignature<'textStyle'>,
  aliasStrategy: CssResolvableAliasStrategy,
  unresolvableAliasStrategy: CssUnresolvableAliasStrategy<UnresolvableReturn> = DEFAULT_UNRESOLVABLE_STRATEGY,
) {
  const variables = textStyle.font
    ...
    .mapUnresolvableValueLevelAlias(alias =>
      unresolvableAliasStrategy(textStyle.font.tokenState, alias),
    )
    .unwrap() as {
    'font-style': string;
    'font-family': string;
  ...
  variables['font-size'] = unwrapInnerValue(
    textStyle.fontSize.mapPrimitiveValue(rawDimensionToCss),
    aliasStrategy,
    unresolvableAliasStrategy,
  );

  const color = unwrapInnerValue(
    textStyle.color.mapPrimitiveValue(v => {
      if (!v) return '';
  ...
  if (color && color !== '') variables['color'] = color;

  const textIndent = unwrapInnerValue(
    textStyle.textIndent.mapPrimitiveValue(v => {
      if (!v) return '';
  ...
  if (textIndent && textIndent !== '') variables['text-indent'] = textIndent;

  const lineHeight = unwrapInnerValue(
    textStyle.lineHeight.mapPrimitiveValue(v => {
      if (!v) return '';
  ...
  if (lineHeight && lineHeight !== '') variables['line-height'] = lineHeight;

  const letterSpacing = unwrapInnerValue(
    textStyle.letterSpacing.mapPrimitiveValue(v => {
      if (!v) return '';
  ...
  if (letterSpacing && letterSpacing !== '') variables['letter-spacing'] = letterSpacing;

  const textAlign = unwrapInnerValue(
    textStyle.textAlignHorizontal,
    aliasStrategy,
    unresolvableAliasStrategy,
  );
  if (textAlign) variables['text-align'] = textAlign;

  const textDecoration = unwrapInnerValue(
    textStyle.textDecoration,
    aliasStrategy,
    unresolvableAliasStrategy,
  );
  if (textDecoration) variables['text-decoration'] = textDecoration;
 ...
 */
export function textStyleValueToCss(textStyle: SpecifyTextStyleValue) {
  const variables = fontValueToCss(textStyle.font) as {
    'font-style': string;
    'font-family': string;
    'font-weight': number;
    'font-size': string;
  ...
  if (!!textStyle.color) variables['color'] = colorValueToCss(textStyle.color);

  if (!!textStyle.textIndent) variables['text-indent'] = dimensionValueToCss(textStyle.textIndent);

  if (!!textStyle.lineHeight) variables['line-height'] = dimensionValueToCss(textStyle.lineHeight);

  if (!!textStyle.letterSpacing)
    variables['letter-spacing'] = dimensionValueToCss(textStyle.letterSpacing);

  if (!!textStyle.textAlignHorizontal) variables['text-align'] = textStyle.textAlignHorizontal;

  if (!!textStyle.textTransform) variables['text-transform'] = textStyle.textTransform;

  if (!!textStyle.textDecoration) variables['text-decoration'] = textStyle.textDecoration;
