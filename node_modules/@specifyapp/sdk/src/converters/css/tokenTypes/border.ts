 ...
 */
export function borderToCss<UnresolvableReturn = never>(
  ...
  return (tokenState: TokenState<'border'>) =>
    tokenState
      ...
      .mapTopLevelValue(value =>
        value
          .mapRawValue(rawValue =>
            rawBorderToCss(rawValue, aliasStrategy, unresolvableAliasStrategy),
          )
 ...
 */
export function rawBorderToCss<UnresolvableReturn = undefined>(
  ...
  return `${unwrapInnerValue(
    border.width.mapPrimitiveValue(rawDimensionToCss),
    aliasStrategy,
    unresolvableAliasStrategy,
  )}${typeof borderStyle === 'string' ? ' ' + borderStyle : ''} ${unwrapInnerValue(
    border.color.mapPrimitiveValue(v => rawColorToCss(v, aliasStrategy, unresolvableAliasStrategy)),
    aliasStrategy,
    unresolvableAliasStrategy,
  )}`;
}
 ...
 */
export function borderValueToCss(border: SpecifyBorderValue) {
  return `${border.width.value}${border.width.unit}${
    typeof border.style === 'string' ? ' ' + border.style : ''
  } ${colorValueToCss(border.color)}`;
}
