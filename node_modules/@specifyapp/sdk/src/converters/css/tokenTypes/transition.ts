 ...
 */
export function transitionToCss<UnresolvableReturn = never>(
  ...
  return (tokenState: TokenState<'transition'>) =>
    tokenState
      ...
      .mapTopLevelValue(value =>
        value
          .mapRawValue(rawValue =>
            rawTransitionToCss(rawValue, aliasStrategy, unresolvableAliasStrategy),
          )
          .mapResolvableModeLevelAlias(aliasStrategy)
          .mapUnresolvableModeLevelAlias(alias => unresolvableAliasStrategy(tokenState, alias))
          .unwrap(),
      )
      .unwrap() as { [mode: string]: ReturnType<typeof transitionValueToCss> };
}

export function modeLevelAliasTransitonToVariables(
...
export function modeLevelUnresolvableAliasTransitonToVariables(
 ...
 */
export function rawTransitionToCss<UnresolvableReturn = never>(
  transition: RawValueSignature<'transition'>,
  aliasStrategy: CssResolvableAliasStrategy,
  unresolvableAliasStrategy: CssUnresolvableAliasStrategy<UnresolvableReturn> = DEFAULT_UNRESOLVABLE_STRATEGY,
) {
  return {
    delay: unwrapInnerValue(
      transition.delay.mapPrimitiveValue(rawDurationToCss),
      aliasStrategy,
      unresolvableAliasStrategy,
    ),
    duration: unwrapInnerValue(
      transition.duration.mapPrimitiveValue(rawDurationToCss),
      aliasStrategy,
      unresolvableAliasStrategy,
    ),
    ['timing-function']: unwrapInnerValue(
      transition.timingFunction.mapPrimitiveValue(timingFunction =>
        Array.isArray(timingFunction)
          ? `cubic-bezier(${timingFunction
              .map(v => v.resolveDeepValue().unwrapValue())
              .join(', ')})`
          : `steps(${timingFunction.stepsCount
              .resolveDeepValue()
              .unwrapValue()}, ${timingFunction.jumpTerm.unwrapValue()})`,
      ),
      aliasStrategy,
      unresolvableAliasStrategy,
    ),
 ...
 */
export function transitionValueToCss(transition: SpecifyTransitionValue) {
  return {
