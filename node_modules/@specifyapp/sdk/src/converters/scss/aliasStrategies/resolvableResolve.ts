 ...
 */
export function createResolveAliasStrategy(): ScssResolvableAliasStrategy {
  return value => {
    if (value instanceof ResolvableTopLevelAlias) {
      return value.tokenState.modes.reduce(
        (acc, curr) => {
        ...
        {} as { [mode: string]: string | number | Record<string, string | number> },
      ) as any;
    } else {
